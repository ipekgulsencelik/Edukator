...
namespace Edukator.PresentationLayer.Controllers
{
    public class StatisticController : Controller
    {
        public IActionResult Index()
        {
            ...
            ViewBag.getProgrammingAlgorithmCoursePrice = context.Courses.Where(x => x.Title == "Programming Algorithm").Select(y => y.Price).FirstOrDefault();
            ...
            ViewBag.avgSoftwareCoursePrice = context.Courses.Where(x => x.CategoryID == (context.Categories.Where(y => y.CategoryName == "Software").Select(z => z.CategoryID).FirstOrDefault())).Average(w => w.Price);
            ViewBag.last3CourseTotalPrice = context.Courses.OrderByDescending(x => x.CourseID).Take(3).Sum(y => y.Price);
            ViewBag.getCourseCategorySoftwareAndReviewMore9Score = context.Courses.Where(x => x.CategoryID == (context.Categories.Where(y => y.CategoryName == "Software").Select(z => z.CategoryID).FirstOrDefault()) && x.Review >= 9).Select(w => w.Title).FirstOrDefault();
